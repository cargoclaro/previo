        ...
        <main className="flex-1 px-4 py-6 container max-w-md mx-auto">
          <section className="space-y-6 animate-slide-up">
            <div className="space-y-2">
              <h2 className="text-2xl font-medium tracking-tight">Cargo Claro </h2>
              <p className="text-muted-foreground">Selecciona un proyecto para comenzar a trabajar</p>
            </div>
            
            <div className="grid gap-4">
              {projects.map(project => <ProjectCard key={project.id} title={project.title} icon={project.icon} description={project.description} onClick={() => navigate(project.path)} />)}
            </div>
          </section>
        </main>
      </div>
